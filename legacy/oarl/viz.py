...
def plot_episode(episode_stats, env_name, ylabel):
    ...
    plt.title('{env}: {label}'.format(
        env=env_name, label=re.sub('[_-]', ' ', ylabel)))

    episode_lengths = np.array([[getattr(e, ylabel)
                                 for e in stats] for stats in episode_stats])
    ...
    if episode_lengths.shape[0] > 1:
        plt.fill_between(range(trial_num), p_top, p_bottom,
